...
move $t2,$zero                      #t2 index
move $t3, $t2                       #t3 couter -->copy of t2 index
memo:

li $t7, 1
bgt $t3, $t7, usingPreStore         #if t3 counter is greater than 1
move $a0, $t3
jal fib

move $t2, $t3                       #t3 couter -->copy of t2 index
...
#lw $t4, 0($t1)
lw $a0, 0($t1)                      # load the value from the array cell
syscall
#move $t5, $a0
...
li $t7, 2
sub $t2, $t3, $t7               #index t2 = n-2

add $t2, $t2, $t2 
add $t2, $t2, $t2 
add $t1, $t2, $s3
lw $t5, 0($t1)                  #load value from previous heap


sub $t2, $t3, $t7               #index t2 = n-1
addi $t2, $t2, 1
add $t2, $t2, $t2 
add $t2, $t2, $t2 
add $t1, $t2, $s3
...
add $t6, $t6, $t5                   #t6 = fib(n-2) + fib(n-1)
...
add $t2, $t2, $t2               # double the index again (now 4x)
add $t1, $t2, $s3               # combine the two components of the address
sw $t6, 0($t1)                  # store the value into the array cell
move $t2, $t3
